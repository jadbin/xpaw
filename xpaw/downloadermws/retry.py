...
class RetryMiddleware:
    ...
    async def handle_response(self, request, response):
        if response.status in self.RETRY_HTTP_STATUS:
            return self._retry(request, "http status={0}".format(response.status))

    async def handle_error(self, request, error):
        if isinstance(error, self.RETRY_ERRORS):
            return self._retry(request, "{0}: {1}".format(type(error), error))
    ...
    def _retry(self, request, reason):
        ...
        if retry_times <= self._max_retry_times:
            ...
            log.info("The request(url={0}) has been retried {1} times, and it will be aborted.".format(request.url, self._max_retry_times))
            raise IgnoreRequest()
